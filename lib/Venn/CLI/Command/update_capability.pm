package Venn::CLI::Command::update_capability;
...
sub run {
    ...
    my $original_obj = $self->get_for_update($path, 'capability');

    my $response = $self->submit_to_api($path, 'PUT', {
        description => $self->description // $original_obj->{description},
        explicit    => $self->explicit    // $original_obj->{explicit},
    });
...
Venn::CLI::Command::update_capability - Updates a capability for providers

=head1 SYNOPSIS

$ venn update_capability <capability> [ --description val --[no]explicit ]
